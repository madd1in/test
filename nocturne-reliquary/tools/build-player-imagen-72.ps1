...
public static class PlayerAtlasTools {
  ...
  public static Rectangle KeepPrimaryComponents(Bitmap frame, int keepPad, out double lowerCenter) {
    lowerCenter = frame.Width / 2.0;
    Rectangle rect = new Rectangle(0, 0, frame.Width, frame.Height);
    BitmapData data = frame.LockBits(rect, ImageLockMode.ReadWrite, PixelFormat.Format32bppPArgb);
    int stride = data.Stride;
    ...
    for (int i = 0; i < components.Count; i++) {
      ...
      double centerPenalty = Math.Abs(((c.MinX + c.MaxX) / 2.0) - frame.Width / 2.0) * 2.5;
      double score = c.Area + ch * 18 + cw * 4 - centerPenalty;
      if (score > bestScore) {
        bestScore = score;
        primaryIndex = i;
      }
...
function Build-Frame {
  ...
  $x1 = [Math]::Floor(($col * $Source.Width) / $Columns) + $SourceInset
  $x2 = [Math]::Floor((($col + 1) * $Source.Width) / $Columns) - $SourceInset
  $y1 = [Math]::Floor(($row * $Source.Height) / $Rows) + $SourceInset
  $y2 = [Math]::Floor((($row + 1) * $Source.Height) / $Rows) - $SourceInset
  ...
  $bounds = [PlayerAtlasTools]::KeepPrimaryComponents($raw, 10, [ref]$lowerCenter)
  if ($bounds.IsEmpty) { return (New-TransparentBitmap $FrameWidth $FrameHeight) }
