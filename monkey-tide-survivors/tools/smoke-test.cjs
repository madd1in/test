...
async function run() {
  ...
  assert(debug.enemyTuning.visualScale >= 1.38 && debug.enemyTuning.hitboxScale >= 1.06 && debug.enemyTuning.animatedVisualBoost >= 1.24 && debug.enemyTuning.minAnimatedVisualHeight >= 120 && debug.enemyTuning.minStaticVisualHeight >= 100 && debug.enemyTuning.secondEliteFromWave <= 3, `Enemy readability/pressure tuning missing: ${JSON.stringify(debug.enemyTuning)}`);
  assert(Object.values(debug.enemyVisualReadability.projectedHeights).every((height) => height >= 104), `Enemy sprites are still too small: ${JSON.stringify(debug.enemyVisualReadability)}`);
  assert(Object.values(debug.enemyVisualReadability.animatedProjectedHeights).every((height) => height >= 120), `Animated enemy sprites are still too small: ${JSON.stringify(debug.enemyVisualReadability)}`);
  assert(debug.enemyVisualReadability.runtimeSourceCrops.profiles.enemyAnimSheet.l >= 6 && debug.enemyVisualReadability.runtimeSourceCrops.profiles.extraEnemies.l >= 12 && debug.enemyVisualReadability.runtimeSourceCrops.typeOverrides.gargoyle.l >= 9 && debug.enemyVisualReadability.sourceProfiles.crab === "enemyAnimSheet", `Runtime enemy safe-crops are missing: ${JSON.stringify(debug.enemyVisualReadability)}`);
