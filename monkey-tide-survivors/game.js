...
const imageSources = {
  ...
  fighterSelect: "assets/sprites/fighter_select_actions_hd.png?v=fighter-select-actions-v1",
  ryuActions: "assets/sprites/ryu_action_sheet_imagen_hd_v12.png?v=ryu-tatsumaki-spin-v4",
  ryuHadokenFx: "assets/sprites/ryu_hadoken_fx_imagen_hd_v7.png?v=ryu-hadoken-v7",
  kenActions: "assets/sprites/ken_action_sheet_imagen_hd_v6.png?v=ken-tatsumaki-spin-v2",
  kenDragonFx: "assets/sprites/ken_dragon_fx_imagen_hd_v4.png?v=ken-dragon-v4",
  guileActions: "assets/sprites/guile_action_sheet_imagen_hd_clean_v2.png?v=guile-action-clean-v2",
  chunLiActions: "assets/sprites/chun_li_action_sheet_imagen_hd_v6.png?v=chun-li-action-v6",
  chunLiProjectiles: "assets/sprites/chun_li_projectile_fx_imagen_hd_v6.png?v=chun-li-projectiles-v6",
...
const KEN_ACTION = {
  ...
  frames: 12,
  fps: 18,
  rowsByAction: { walk: 0, stepKick: 1, hadoken: 1, shoryuken: 2, dragonPunch: 2, tatsuKick: 3, whirlwindKick: 3, dragonKick: 4 },
};
const GUILE_ACTION = {
  w: 256,
  h: 256,
...
const STREET_FIGHTER_BASIC_MOVEMENT = {
  ...
  ken: { skinId: "ken", base: "walk", ambient: ["stepKick", "hadoken", "shoryuken", "hadoken", "dragonPunch", "whirlwindKick", "dragonKick"], ambientEvery: 2.25, ambientDuration: 0.54 },
...
const STREET_FIGHTER_STARTER_MOVES = {
  ryu: "hadoken",
  ken: "stepKick",
  guile: "sonicBoom",
  chunLi: "thousandKick",
...
const STREET_FIGHTER_IDLE_NEARBY_RADIUS = 420;
const STREET_FIGHTER_IDLE_FIREBALL_DELAY = { min: 5.6, spread: 2.7 };
const STREET_FIGHTER_IDLE_FRAMES = [0, 1, 0, 2];
const STREET_FIGHTER_FLOW_SKINS = ["ryu", "ken", "guile", "chunLi"];
const STREET_FIGHTER_FLOW_THRESHOLD = 3;
const STREET_FIGHTER_FLOW_MAX_CHARGES = 2;
const STREET_FIGHTER_EX_TUNING = {
  size: 1.2,
  radius: 1.16,
  speed: 1.09,
  damage: 1.24,
...
const KEN_DRAGON_FX = { w: 256, h: 256, cols: 12, rows: 3, frames: 12, fps: 18, rowsByFx: { stepKick: 0, shoryuken: 1, dragonPunch: 1, tatsuKick: 2, whirlwindKick: 2, dragonKick: 2 } };
...
const playerSkinMap = {
  ...
  chunLi: {
    ...
    trait: {
      ...
      desc: "Startet nur mit Thousand Kick; Ki Kou Ken, Whirlwind und Lightning Kick kommen spaeter dazu.",
      speed: 18,
      cooldown: -0.035,
      maxHp: -10,
...
function kiKouKenStage(level = 1) {
  return clamp(Math.floor((Math.max(1, level) - 1) / 3), 0, CHUN_LI_PROJECTILE_FX.rows - 1);
}

function stagedProjectileValue(values, stage) {
  return values[clamp(stage, 0, values.length - 1)] ?? values[0];
}

const FIREBALL_STAGE_TUNING = {
  hadoken: {
    size: [82, 100, 118, 142],
    radius: [21, 26, 31, 38],
  },
...
function horizontalProjectileAngle(referenceAngle = 0) {
  const x = Math.cos(Number.isFinite(referenceAngle) ? referenceAngle : 0);
  const fallback = Math.sign(state?.player?.facing || 1) || 1;
  const dir = Math.abs(x) > 0.12 ? Math.sign(x) : fallback;
...
function soundVolume(key, gain = 1) {
  return clamp((soundConfig[key]?.volume ?? 0.16) * SFX_MASTER_GAIN * gain, 0, 1);
}
...
function isKenSkin(skinId = state?.player?.skin || selectedSkin) {
  return skinId === "ken";
}
...
function actionConfigForSkin(skinId = state?.player?.skin || selectedSkin) {
  ...
  if (isKenSkin(skinId)) return KEN_ACTION;
...
function kenActionFrame(action) {
  return actionFrame(action, KEN_ACTION);
}
...
function streetFighterMoveLevel(skinId = streetFighterSkinId()) {
  if (!skinId) return 1;
  return Math.max(1, state?.weapons?.cutlass?.level || 1);
}

function streetFighterMoveUnlocked(skinId, move, level = streetFighterMoveLevel(skinId)) {
  if (!skinId || !move) return true;
  const unlocks = STREET_FIGHTER_MOVE_UNLOCKS[skinId];
  if (!unlocks) return true;
  return level >= (unlocks[move] || 1);
}

function streetFighterActionsForProfile(profile, level = streetFighterMoveLevel(profile?.skinId)) {
  const actions = [...(profile?.ambient || [])];
  const skinId = profile?.skinId;
  if (!skinId) return actions;
  const unlocked = actions.filter((action) => streetFighterMoveUnlocked(skinId, action, level));
  return unlocked.length ? unlocked : [STREET_FIGHTER_STARTER_MOVES[skinId]].filter(Boolean);
}
...
function basicMovementFrame(config, profile, rowForAction, moving) {
  ...
  const quietIdle = !moving && streetFighterPeacefulIdle(movement.skinId);
  const idleFrames = movement.idleFrames || STREET_FIGHTER_IDLE_FRAMES;
  const fps = quietIdle ? 3.8 : (moving ? config.fps : Math.max(6, config.fps * 0.42)) || 12;
  const elapsed = state?.elapsed || 0;
  let action = baseAction;
  let frame = quietIdle
    ? idleFrames[positiveModulo(Math.floor(elapsed * fps), idleFrames.length)] || 0
    : positiveModulo(Math.floor(elapsed * fps), config.frames);
  const ambientActions = streetFighterActionsForProfile(movement);
  if (!moving && !quietIdle && ambientActions.length) {
    ...
    if (cycleTime < duration) {
      action = ambientActions[Math.floor(elapsed / every) % ambientActions.length] || baseAction;
...
function basicMovementSummary(config, profile) {
  ...
  const level = streetFighterMoveLevel(movement.skinId);
  const ambientActions = streetFighterActionsForProfile(movement, level);
  const startupActions = streetFighterActionsForProfile(movement, 1);
  return {
    rows: 1,
    framesPerRow: config.frames,
    totalFrames: config.frames,
    baseAction: movement.base || "walk",
    skinId: movement.skinId || null,
    moveLevel: level,
    ambientActions,
    startupActions,
    allAmbientActions: [...(movement.ambient || [])],
    unlocks: movement.skinId ? { ...STREET_FIGHTER_MOVE_UNLOCKS[movement.skinId] } : {},
    idleFrames: [...(movement.idleFrames || STREET_FIGHTER_IDLE_FRAMES)],
    peacefulIdleOnly: true,
    peacefulIdleNearbyRadius: STREET_FIGHTER_IDLE_NEARBY_RADIUS,
    ambientEvery,
    ambientDuration,
...
function kenActionRow(type = "walk") {
  return KEN_ACTION.rowsByAction[type] ?? KEN_ACTION.rowsByAction.walk;
}
...
function weaponPresentation(id, skinId = state?.player?.skin || selectedSkin) {
  ...
  if (id === "cutlass" && isKenSkin(skinId)) {
    return {
      ...
      name: "Dragon Rush",
      icon: "shoryuken",
      desc: "Ken startet nur mit Step Kick; Hadoken, Dragon Punch, Tatsu und Dragon Kick sind Upgrade-Stufen.",
    };
  }
  if (id === "cutlass" && isChunLiSkin(skinId)) {
    return {
      ...
      desc: "Chun Li startet nur mit Thousand Kick; Ki Kou Ken, Whirlwind und Lightning Kick bauen darauf auf.",
    };
  }
...
function upgradePresentation(upgrade) {
  ...
  if (upgrade.id === "cutlass" && isKenSkin()) {
    return {
      ...
      icon: "shoryuken",
      desc: "Levelt Kens Step-Kick-Start in Hadoken, Dragon Punch, Whirlwind Kick und Dragon Kick hinein.",
    };
  }
  if (upgrade.id === "cutlass" && isChunLiSkin()) {
    return {
      ...
      icon: "thousandKickArc",
      desc: "Levelt Chun Lis Thousand-Kick-Start in Ki Kou Ken, Whirlwind und Lightning Kick hinein.",
    };
  }
  return upgrade;
}
...
function applyCharacterTrait(next) {
  ...
  if (streetFighterSkinId(next.player.skin)) {
    for (const weapon of ["coconut", "compass", "bottle", "rope"]) {
      if (!next.weapons[weapon]) continue;
      next.weapons[weapon].level = 0;
      next.upgradeCounts[weapon] = 0;
    }
    if (next.weapons.compass?.hits) next.weapons.compass.hits.clear();
  }
}
...
function makeState() {
  ...
  const next = {
    ...
    streetFighterFlow: {
      chain: Object.fromEntries(STREET_FIGHTER_FLOW_SKINS.map((id) => [id, 0])),
      charges: Object.fromEntries(STREET_FIGHTER_FLOW_SKINS.map((id) => [id, 0])),
      totalEx: Object.fromEntries(STREET_FIGHTER_FLOW_SKINS.map((id) => [id, 0])),
      lastSkin: null,
      lastMove: null,
      readyMove: null,
    },
    ryuArsenal: { casts: 0, hadokens: 0, weaponHadokens: 0, shoryukens: 0, whirlwindKicks: 0 },
    kenArsenal: { casts: 0, stepKicks: 0, hadokens: 0, shoryukens: 0, dragonPunches: 0, tatsuKicks: 0, whirlwindKicks: 0, dragonKicks: 0 },
...
function spriteSheetCellStyle(src, config, row, frame) {
  const safeFrame = clamp(frame || 0, 0, Math.max(0, config.frames - 1));
  const safeRow = clamp(row || 0, 0, Math.max(0, config.rows - 1));
  const x = safeFrame / Math.max(1, config.frames - 1) * 100;
  const y = safeRow / Math.max(1, config.rows - 1) * 100;
  return `background-image:url('${src}');background-size:${config.frames * 100}% ${config.rows * 100}%;background-position:${x}% ${y}%;background-repeat:no-repeat;`;
}

function streetFighterSkinIconStyle(id) {
  const actionSelectIndex = { ryu: 0, ken: 1, guile: 2, chunLi: 3 }[id];
  if (actionSelectIndex !== undefined) {
    const x = actionSelectIndex / Math.max(1, FIGHTER_SELECT.cols - 1) * 100;
    return `background-image:url('${imageSources.fighterSelect}');background-size:${FIGHTER_SELECT.cols * 100}% 100%;background-position:${x}% 50%;background-repeat:no-repeat;`;
  }
  return null;
}
...
function skinIconStyle(id) {
  ...
  const fighterActionStyle = streetFighterSkinIconStyle(id);
  if (fighterActionStyle) return fighterActionStyle;
...
function renderSkinPicker() {
  ...
  ui.skinPicker.innerHTML = playerSkinIds.map((id) => {
    ...
    const isFighter = skin?.animSheet === "fighterWalks" || skin?.animSheet === "ryuActions" || skin?.animSheet === "kenActions" || skin?.animSheet === "guileActions" || skin?.animSheet === "chunLiActions";
...
function updateWeapons(dt) {
  ...
  if (w.cutlass.timer <= 0) {
    ...
    const baseCooldown = isGuileSkin() ? 0.72 : isRyuSkin() ? 0.66 : isKenSkin() ? 0.5 : isChunLiSkin() ? 0.54 : 0.6;
    const cooldown = Math.max(0.18, (baseCooldown - lvl * 0.05) * cooldownMult);
    w.cutlass.timer = cooldown;
    if (!streetFighterSkinId() || !streetFighterPeacefulIdle()) {
...
function updateSignatureMove(dt) {
  ...
  if (streetFighterPeacefulIdle()) {
    state.signatureMove.timer = 0.45;
    return;
  }
...
function ensureKenArsenal() {
  ...
  if (state.kenArsenal.whirlwindKicks === undefined) state.kenArsenal.whirlwindKicks = 0;
...
function ensureStreetFighterFlowState() {
  if (!state.streetFighterFlow) state.streetFighterFlow = {};
  const flow = state.streetFighterFlow;
  if (!flow.chain) flow.chain = {};
  if (!flow.charges) flow.charges = {};
  if (!flow.totalEx) flow.totalEx = {};
  for (const id of STREET_FIGHTER_FLOW_SKINS) {
    if (flow.chain[id] === undefined) flow.chain[id] = 0;
    if (flow.charges[id] === undefined) flow.charges[id] = 0;
    if (flow.totalEx[id] === undefined) flow.totalEx[id] = 0;
  }
  if (flow.lastSkin === undefined) flow.lastSkin = null;
  if (flow.lastMove === undefined) flow.lastMove = null;
  if (flow.readyMove === undefined) flow.readyMove = null;
  return flow;
}

function streetFighterFlowSkin(skinId = streetFighterSkinId()) {
...
function registerStreetFighterFlowMove(skinId = streetFighterSkinId(), move = "signature", options = {}) {
  ...
  flow.lastSkin = id;
  flow.lastMove = move;
  if (flow.chain[id] < STREET_FIGHTER_FLOW_THRESHOLD) return false;
  flow.chain[id] = 0;
...
function markStreetFighterExFlash(skinId, move, x, y, radius = 72) {
  if (!state?.zones) return;
  state.zones.push({
    type: "streetFighterExFlash",
    skinId,
    move,
    x,
    y,
    radius: Math.max(96, radius * 2.2),
    life: 0.34,
    maxLife: 0.34,
  });
  floatingText("EX", x, y - Math.max(56, radius * 0.65), "#fff2a8", 0.58, 24, { priority: 1 });
}

function consumeStreetFighterEx(skinId = streetFighterSkinId(), move = "signature", origin = {}, options = {}) {
  const id = streetFighterFlowSkin(skinId);
  if (!id || options.noEx || options.ex === false) return null;
  const flow = ensureStreetFighterFlowState();
  if ((flow.charges[id] || 0) <= 0) return null;
  flow.charges[id] -= 1;
  flow.totalEx[id] = (flow.totalEx[id] || 0) + 1;
  markStreetFighterExFlash(id, move, origin.x ?? state.player.x, origin.y ?? state.player.y, origin.radius || 72);
  return { ...STREET_FIGHTER_EX_TUNING, skinId: id, move, count: flow.totalEx[id] };
}

function streetFighterExProjectileStats(stats, ex) {
  if (!ex) return { ...stats, ex: false };
  return {
    ...stats,
    size: stats.size * ex.size,
    radius: stats.radius * ex.radius,
    speed: stats.speed * ex.speed,
    damage: stats.damage * ex.damage,
    life: stats.life + ex.life,
    pierce: stats.pierce + ex.pierce,
    ex: true,
  };
}
...
function castSignatureMove(signature) {
  ...
  const fighterSkin = streetFighterSkinId();
  const fighterLevel = streetFighterMoveLevel(fighterSkin);
  if (signature.id === "shoryuken" && isKenSkin()) {
    ...
    if (level >= 5 && !["tatsuKick", "whirlwindKick"].includes(mainMove) && castIndex % 5 === 0) castKenWhirlwindKick(signature, angle, { showPlayerAction: false });
  ...
  if (signature.id === "kiKouKen" && isChunLiSkin() && !streetFighterMoveUnlocked("chunLi", "kiKouKen", fighterLevel)) {
    const arsenal = ensureChunLiArsenal();
    arsenal.casts += 1;
    castChunLiThousandKick(fighterLevel, angle);
    state.signatureMove.casts += 1;
    state.signatureMove.last = "thousandKick";
    floatingText("Thousand Kick", p.x, p.y - 112, signature.color || "#fff2c7", 0.72, 22, { priority: 2 });
    playSkinSound("powerup", "upgradeMagic", { cooldown: 1200 });
    return true;
  }
  if (signature.id === "sonicBoom") {
    ...
    arsenal.casts += 1;
    arsenal.hadokens += 1;
  } else if (signature.id === "dragonKick" && isKenSkin()) {
    const arsenal = ensureKenArsenal();
    triggerPlayerAction("dragonKick", 0.74);
    arsenal.casts += 1;
  ...
  if (signature.id === "dragonKick" && isKenSkin()) {
    const arsenal = ensureKenArsenal();
    if (arsenal.casts % 2 === 0) castKenDragonPunch(signature, angle);
    if (arsenal.casts % 3 === 0) castKenTatsuKick(signature, angle);
  }
  if (signature.id === "kiKouKen" && isChunLiSkin()) {
    const arsenal = ensureChunLiArsenal();
    const level = streetFighterMoveLevel("chunLi");
    if (streetFighterMoveUnlocked("chunLi", "whirlwindKick", level) && arsenal.casts % 3 === 0) castChunLiWhirlwindKick(signature, angle);
    if (streetFighterMoveUnlocked("chunLi", "lightningKick", level) && arsenal.casts % 5 === 0) castChunLiLightningKick(signature, angle);
  }
  floatingText(signature.label || "Signature", p.x, p.y - 112, signature.color || "#fff2c7", 0.72, 22, { priority: 2 });
  playSkinSound("powerup", "upgradeMagic", { cooldown: 1200 });
  return true;
...
function fireSignatureProjectile(signature, angle) {
  const p = state.player;
  if (signature.id === "sonicBoom") {
    const shotAngle = horizontalProjectileAngle(angle);
    const level = Math.max(1, state.weapons.cutlass.level || 1);
    const stage = sonicBoomStage(level);
    registerStreetFighterFlowMove("guile", "sonicBoom");
    const baseStats = {
      size: stagedProjectileValue(FIREBALL_STAGE_TUNING.sonicBoom.size, stage),
      radius: stagedProjectileValue(FIREBALL_STAGE_TUNING.sonicBoom.radius, stage),
      speed: Math.max(signature.speed || 560, 690 + level * 18 + stage * 34),
    ...
    const stats = streetFighterExProjectileStats(baseStats, consumeStreetFighterEx("guile", "sonicBoom", { x: p.x, y: p.y - 8, radius: baseStats.radius }));
    state.projectiles.push({
      ...
      y: p.y - 8,
      vx: Math.cos(shotAngle) * stats.speed,
      vy: 0,
      r: stats.radius,
      damage: stats.damage,
      ...
      size: stats.size,
      ex: stats.ex,
      exMove: stats.ex ? "sonicBoom" : null,
      retarget: false,
    });
    return;
  }
  if (signature.id === "hadoken" && isRyuSkin()) {
    const focus = Math.max(1, Math.min(7, state.weapons.cutlass.level || 1));
    fireRyuHadoken(focus, angle, { signature, countArsenal: false, action: false, signatureCast: true });
    return;
  }
  if (signature.id === "kiKouKen" && isChunLiSkin()) {
    const shotAngle = horizontalProjectileAngle(angle);
    const focus = Math.max(1, Math.min(8, state.weapons.cutlass.level || 1));
    const stage = kiKouKenStage(focus);
    registerStreetFighterFlowMove("chunLi", "kiKouKen");
    const baseStats = {
      size: stagedProjectileValue(FIREBALL_STAGE_TUNING.kiKouKen.size, stage),
      radius: stagedProjectileValue(FIREBALL_STAGE_TUNING.kiKouKen.radius, stage),
      speed: Math.max(signature.speed || 620, 760 + focus * 18),
      damage: (signature.damage || 30) + focus * 4,
      life: 1.72 + focus * 0.05,
      pierce: Math.max(signature.pierce ?? 1, 3 + Math.floor(focus / 2)),
    };
    const stats = streetFighterExProjectileStats(baseStats, consumeStreetFighterEx("chunLi", "kiKouKen", { x: p.x, y: p.y - 8, radius: baseStats.radius }));
    state.projectiles.push({
      type: "signature",
      icon: "kiKouKen",
...
function fireRyuHadoken(level, fallbackAngle = 0, options = {}) {
  ...
  const target = nearestEnemy();
  const aimAngle = options.exactAngle === false && target ? Math.atan2(target.y - p.y, target.x - p.x) : (target && !options.signatureCast ? Math.atan2(target.y - p.y, target.x - p.x) : fallbackAngle);
  const angle = horizontalProjectileAngle(aimAngle);
  const safeLevel = Math.max(1, level || 1);
  const stage = ryuHadokenStage(safeLevel);
  registerStreetFighterFlowMove("ryu", "hadoken", options);
  const baseStats = {
    size: stagedProjectileValue(FIREBALL_STAGE_TUNING.hadoken.size, stage),
    radius: stagedProjectileValue(FIREBALL_STAGE_TUNING.hadoken.radius, stage),
    speed: Math.max(signature.speed || 620, 720 + safeLevel * 36 + stage * 40),
  ...
  const stats = streetFighterExProjectileStats(baseStats, consumeStreetFighterEx("ryu", "hadoken", { x: p.x, y: p.y - 8, radius: baseStats.radius }, options));
  ...
  for (let i = 0; i < count; i += 1) {
    const laneOffset = count > 1 ? (i - (count - 1) / 2) * 22 : 0;
    state.projectiles.push({
      type: "signature",
      icon: "hadoken",
      signatureId: "hadoken",
...
function fireKenHadoken(level, fallbackAngle = 0, signature = state.characterTrait?.signature || {}, options = {}) {
  ...
  const safeLevel = Math.max(1, level || 1);
  const stage = ryuHadokenStage(safeLevel);
  registerStreetFighterFlowMove("ken", "hadoken", options);
  const baseStats = {
    size: stagedProjectileValue(FIREBALL_STAGE_TUNING.hadoken.size, stage) * 0.94,
    radius: stagedProjectileValue(FIREBALL_STAGE_TUNING.hadoken.radius, stage) * 0.92,
    speed: Math.max(signature.speed || 620, 700 + safeLevel * 30 + stage * 36),
  ...
  const stats = streetFighterExProjectileStats(baseStats, consumeStreetFighterEx("ken", "hadoken", { x: p.x, y: p.y - 8, radius: baseStats.radius }, options));
  const arsenal = ensureKenArsenal();
  arsenal.hadokens += 1;
...
function kenComboChoice(level, castIndex, signatureCast = false) {
  ...
  const sequences = signatureCast
    ? safeLevel >= 6
      ? ["hadoken", "shoryuken", "hadoken", "dragonPunch", "hadoken", "shoryuken", "whirlwindKick", "dragonKick"]
      : safeLevel >= 5
        ? ["hadoken", "shoryuken", "hadoken", "dragonPunch", "whirlwindKick"]
        : safeLevel >= 4
          ...
          : safeLevel >= 3
            ...
            : safeLevel >= 2
              ...
              : ["stepKick"]
    : safeLevel >= 6
      ? ["stepKick", "hadoken", "shoryuken", "hadoken", "dragonPunch", "hadoken", "whirlwindKick", "dragonKick", "shoryuken", "hadoken"]
      : safeLevel >= 5
        ? ["stepKick", "hadoken", "shoryuken", "hadoken", "dragonPunch", "whirlwindKick", "hadoken"]
        : safeLevel >= 4
...
function castKenComboMove(move, level, angle, signature, options = {}) {
  ...
  if (move === "tatsuKick" || move === "whirlwindKick") {
    const isWhirlwind = move === "whirlwindKick";
    castKenTatsuKick(signature, angle, isWhirlwind ? { ...options, move: "whirlwindKick", fx: "whirlwindKick" } : options);
    return move;
  }
  if (move === "dragonKick") {
...
function castKenShoryuken(level, fallbackAngle = 0) {
  ...
  if (level >= 6 && !["tatsuKick", "whirlwindKick"].includes(mainMove) && castIndex % 8 === 0) castKenWhirlwindKick(signature, angle, { showPlayerAction: false });
}
...
function castKenStepKick(level, fallbackAngle = 0, options = {}) {
  ...
  const angle = target ? Math.atan2(target.y - p.y, target.x - p.x) : fallbackAngle;
  arsenal.stepKicks += 1;
  registerStreetFighterFlowMove("ken", "stepKick", options);
  ...
  for (const enemy of state.enemies) {
    ...
    if (dist <= reach + enemy.r && delta < 0.82) {
      hurtEnemy(enemy, (21 + level * 8) * state.stats.damage, Math.cos(angle) * 1.1, Math.sin(angle));
    }
  }
...
function castKenDragonPunch(signature, angle, options = {}) {
  ...
  registerStreetFighterFlowMove("ken", move, options);
  ...
  for (const enemy of state.enemies) {
    ...
    const dist = Math.hypot(dx, dy);
    if (dist <= radius + enemy.r) {
      hurtEnemy(enemy, (signature.damage || 34) * 0.86 * state.stats.damage * (state.stats.signatureDamage || 1), dx / Math.max(1, dist), -1.2);
      enemy.y -= 22;
    }
  }
  shake(0.32);
...
function castKenTatsuKick(signature, angle, options = {}) {
  const p = state.player;
  const arsenal = ensureKenArsenal();
  const move = options.move || "tatsuKick";
  const fx = options.fx || move;
  arsenal.tatsuKicks += 1;
  if (move === "whirlwindKick") arsenal.whirlwindKicks += 1;
  registerStreetFighterFlowMove("ken", move, options);
  if (options.showPlayerAction !== false) triggerPlayerAction(move, 0.76);
  const radius = 118 + Math.min(40, state.level * 4);
  state.zones.push({ type: "kenWhirlwind", move, fx, x: p.x, y: p.y - 14, angle, radius, life: 0.42, maxLife: 0.42, signature: signature.id, spinKick: true, spinRate: 5.6 });
  for (const enemy of state.enemies) {
    const dx = enemy.x - p.x;
    const dy = enemy.y - p.y;
    const dist = Math.hypot(dx, dy);
    if (dist <= radius + enemy.r) {
      const tangentX = -dy / Math.max(1, dist);
      const tangentY = dx / Math.max(1, dist);
      hurtEnemy(enemy, (signature.damage || 34) * 0.64 * state.stats.damage * (state.stats.signatureDamage || 1), tangentX * 0.86, tangentY * 0.86);
      enemy.x += tangentX * 20;
      enemy.y += tangentY * 20;
    }
  }
...
function castKenWhirlwindKick(signature, angle, options = {}) {
  castKenTatsuKick(signature, angle, { ...options, move: "whirlwindKick", fx: "whirlwindKick" });
}
...
function castKenDragonKick(signature, angle, options = {}) {
  ...
  const arsenal = ensureKenArsenal();
  arsenal.dragonKicks += 1;
  registerStreetFighterFlowMove("ken", "dragonKick", options);
...
function castChunLiThousandKick(level, fallbackAngle = 0) {
  ...
  registerStreetFighterFlowMove("chunLi", "thousandKick");
...
function castChunLiWhirlwindKick(signature, angle) {
  ...
  const arsenal = ensureChunLiArsenal();
  arsenal.whirlwindKicks += 1;
  registerStreetFighterFlowMove("chunLi", "whirlwindKick");
  triggerPlayerAction("whirlwindKick", 0.78);
...
function castChunLiLightningKick(signature, angle) {
  ...
  registerStreetFighterFlowMove("chunLi", "lightningKick");
...
function streetFighterStandingStill() {
  const p = state?.player;
  if (!p) return false;
  return Math.hypot(p.moveX || 0, p.moveY || 0) <= 0.05 && (p.dash || 0) <= 0;
}

function streetFighterHasNearbyEnemy(radius = STREET_FIGHTER_IDLE_NEARBY_RADIUS) {
  const p = state?.player;
  if (!p || !state?.enemies) return false;
  for (const enemy of state.enemies) {
    if (!enemy || enemy.hp <= 0) continue;
    if (Math.hypot(enemy.x - p.x, enemy.y - p.y) <= radius + (enemy.r || 0)) return true;
  }
  return false;
}

function streetFighterPeacefulIdle(skinId = streetFighterSkinId()) {
  return !!skinId && state?.phase === "playing" && streetFighterStandingStill() && !streetFighterHasNearbyEnemy();
}

function nextStreetFighterIdleFireballDelay() {
  return STREET_FIGHTER_IDLE_FIREBALL_DELAY.min + Math.random() * STREET_FIGHTER_IDLE_FIREBALL_DELAY.spread;
}
...
function ensureStreetFighterIdleState() {
  ...
  if (!state.streetFighterIdle.fireballTimers) state.streetFighterIdle.fireballTimers = {};
  if (!state.streetFighterIdle.fireballs) state.streetFighterIdle.fireballs = { ryu: 0, ken: 0, guile: 0, chunLi: 0 };
  for (const id of ["ryu", "ken", "guile", "chunLi"]) {
    ...
    if (state.streetFighterIdle.fireballs[id] === undefined) state.streetFighterIdle.fireballs[id] = 0;
    if (state.streetFighterIdle.fireballTimers[id] === undefined) state.streetFighterIdle.fireballTimers[id] = nextStreetFighterIdleFireballDelay();
  }
...
function updateStreetFighterIdleState(dt) {
  ...
  if (!state?.player || !skinId) return;
  const idle = ensureStreetFighterIdleState();
  idle.contactCooldown = Math.max(0, idle.contactCooldown - dt);
  if (!streetFighterPeacefulIdle(skinId) || state.playerAction) {
    idle.fireballTimers[skinId] = Math.max(idle.fireballTimers[skinId] || 0, 1.2);
    return;
  }
  idle.fireballTimers[skinId] -= dt;
  if (idle.fireballTimers[skinId] > 0) return;
  if (castStreetFighterIdleFireball(skinId)) {
    idle.fireballs[skinId] = (idle.fireballs[skinId] || 0) + 1;
  }
  idle.fireballTimers[skinId] = nextStreetFighterIdleFireballDelay();
}

function castStreetFighterIdleFireball(skinId = streetFighterSkinId()) {
  const p = state?.player;
  if (!p || !skinId) return false;
  const level = streetFighterMoveLevel(skinId);
  const angle = p.facing < 0 ? Math.PI : 0;
  const signature = state.characterTrait?.signature || characterTrait(skinId)?.signature || { id: "hadoken", damage: 30, radius: 22, pierce: 2 };
  if (skinId === "ryu" && streetFighterMoveUnlocked("ryu", "hadoken", level)) {
    fireRyuHadoken(level, angle, { signature, exactAngle: true, signatureCast: true });
    return true;
  }
  if (skinId === "ken" && streetFighterMoveUnlocked("ken", "hadoken", level)) {
    fireKenHadoken(level, angle, signature, { exactAngle: true, signatureCast: true });
    return true;
  }
  if (skinId === "guile" && streetFighterMoveUnlocked("guile", "sonicBoom", level)) {
    fireGuileSonicBoom(level, angle, { exactAngle: true, followups: false });
    return true;
  }
  if (skinId === "chunLi" && streetFighterMoveUnlocked("chunLi", "kiKouKen", level)) {
    const arsenal = ensureChunLiArsenal();
    arsenal.casts += 1;
    arsenal.kiKouKens += 1;
    triggerPlayerAction("kiKouKen", 0.62);
    fireSignatureProjectile({ ...signature, id: "kiKouKen", icon: "kiKouKen" }, angle);
    playSkinSound("hit", "quickCutlass", { cooldown: 420 });
    return true;
  }
  return false;
}
...
function triggerStreetFighterContactCounter(enemy, dx, dy, dist) {
  ...
  if (skinId === "ryu") {
    if (streetFighterMoveUnlocked("ryu", "whirlwindKick", level) && count % 3 === 0) castRyuWhirlwindKick(signature, angle);
    ...
    const flowReady = streetFighterMoveUnlocked("ken", "hadoken", level)
      && streetFighterMoveUnlocked("ken", "shoryuken", level)
      && streetFighterMoveUnlocked("ken", "whirlwindKick", level);
    const flowMove = flowReady ? ["hadoken", "shoryuken", "whirlwindKick"][positiveModulo(count - 1, 3)] : null;
    if (flowMove === "hadoken") fireKenHadoken(level, angle, signature, { exactAngle: true, signatureCast: true });
    else if (flowMove === "shoryuken") castKenDragonPunch(signature, angle, { move: "shoryuken", fx: "shoryuken", level });
    else if (flowMove === "whirlwindKick") castKenWhirlwindKick(signature, angle);
    else if (streetFighterMoveUnlocked("ken", "shoryuken", level)) castKenDragonPunch(signature, angle, { move: "shoryuken", fx: "shoryuken", level });
    else if (streetFighterMoveUnlocked("ken", "hadoken", level)) fireKenHadoken(level, angle, signature, { exactAngle: true, signatureCast: true });
    ...
    else if (streetFighterMoveUnlocked("guile", "reversePunch", level) && count % 2 === 0) castGuileReversePunch(level, angle);
    else if (streetFighterMoveUnlocked("guile", "kneeBazooka", level)) castGuileKneeBazooka(level, angle);
    else fireGuileSonicBoom(level, angle);
  } else if (skinId === "chunLi") {
    if (streetFighterMoveUnlocked("chunLi", "whirlwindKick", level) && count % 3 === 0) castChunLiWhirlwindKick(signature, angle);
    else if (streetFighterMoveUnlocked("chunLi", "lightningKick", level) && count % 2 === 0) castChunLiLightningKick(signature, angle);
...
function fireGuileSonicBoom(level, fallbackAngle = 0, options = {}) {
  const p = state.player;
  if (!state.guileArsenal) state.guileArsenal = { casts: 0, sonicBooms: 0, kneeBazookas: 0, reversePunches: 0, flashKicks: 0 };
  const target = options.exactAngle ? null : nearestEnemy();
  const aimAngle = target ? Math.atan2(target.y - p.y, target.x - p.x) : fallbackAngle;
  const angle = horizontalProjectileAngle(aimAngle);
  const stage = sonicBoomStage(level);
  registerStreetFighterFlowMove("guile", "sonicBoom", options);
  const baseStats = {
    size: stagedProjectileValue(FIREBALL_STAGE_TUNING.sonicBoom.size, stage),
    radius: stagedProjectileValue(FIREBALL_STAGE_TUNING.sonicBoom.radius, stage),
    speed: 690 + level * 30 + stage * 36,
    damage: 28 + level * 9 + stage * 4,
    life: 1.45 + level * 0.11 + stage * 0.08,
    pierce: 2 + stage + Math.floor(level / 2),
  };
  const stats = streetFighterExProjectileStats(baseStats, consumeStreetFighterEx("guile", "sonicBoom", { x: p.x, y: p.y - 10, radius: baseStats.radius }, options));
  const count = level >= 7 ? 2 : 1;
  state.guileArsenal.sonicBooms += count;
  state.guileArsenal.casts += 1;
  if (options.showPlayerAction !== false) triggerPlayerAction("sonicBoom", 0.5);
  for (let i = 0; i < count; i += 1) {
    const laneOffset = count > 1 ? (i - (count - 1) / 2) * 22 : 0;
    state.projectiles.push({
      type: "signature",
  ...
  if (options.followups !== false) {
    if (level >= 2 && state.guileArsenal.casts % 3 === 0) castGuileKneeBazooka(level, aimAngle);
    if (level >= 4 && state.guileArsenal.casts % 5 === 0) castGuileReversePunch(level, aimAngle);
...
function castGuileKneeBazooka(level, angle) {
  ...
  registerStreetFighterFlowMove("guile", "kneeBazooka");
...
function castGuileReversePunch(level, angle) {
  ...
  registerStreetFighterFlowMove("guile", "reversePunch");
...
function castGuileFlashKick(level) {
  ...
  registerStreetFighterFlowMove("guile", "flashKick");
...
function spawnSonicBoomBurst(projectile) {
  ...
  state.zones.push({
    ...
    radius: (projectile.r || 24) * (projectile.ex ? 2.15 : 1.8 + stage * 0.18),
    life: projectile.ex ? 0.26 : 0.2,
    maxLife: projectile.ex ? 0.26 : 0.2,
    stage,
    ex: !!projectile.ex,
...
function spawnRyuHadokenBurst(projectile) {
  ...
  state.zones.push({
    ...
    y: projectile.y,
    angle: Math.atan2(projectile.vy || 0, projectile.vx || 1),
    radius: (projectile.r || 24) * (projectile.ex ? 2.2 : 1.85 + stage * 0.2),
    life: projectile.ex ? 0.28 : 0.22,
    maxLife: projectile.ex ? 0.28 : 0.22,
    stage,
    ex: !!projectile.ex,
    frameSeed: projectile.animSeed || 0,
  });
...
function spawnChunLiProjectileBurst(projectile) {
  state.zones.push({
    ...
    y: projectile.y,
    angle: Math.atan2(projectile.vy || 0, projectile.vx || 1),
    radius: (projectile.r || 24) * (projectile.ex ? 2.26 : 1.95),
    life: projectile.ex ? 0.28 : 0.22,
    maxLife: projectile.ex ? 0.28 : 0.22,
    ex: !!projectile.ex,
    frameSeed: projectile.animSeed || 0,
...
function drawPlayer() {
  ...
  if (skin.animSheet === "samMaxDuoWalk" && images.samMaxDuoWalk) {
    ...
    const bob = action ? 0 : moving ? 0 : Math.sin(state.elapsed * 3.2) * 0.9;
    const actionScale = action?.type === "dragonKick" ? 1.2 : action?.type === "shoryuken" || action?.type === "dragonPunch" || action?.type === "tatsuKick" || action?.type === "whirlwindKick" ? 1.14 : action ? 1.08 : 1;
    const h = skin.animH * actionScale;
    const w = h;
...
function drawProjectiles() {
  ...
  for (const projectile of state.projectiles) {
    ...
    if (projectile.signatureId === "sonicBoom" && images.sonicBoomFx) {
      ...
      drawStreetFighterExProjectileHalo(projectile, ox + projectile.x, oy + projectile.y, size);
...
function drawStreetFighterExProjectileHalo(projectile, x, y, size) {
  if (!projectile.ex) return;
  ctx.save();
  ctx.translate(x, y);
  ctx.rotate((projectile.spin || 0) + state.elapsed * 1.2);
  ctx.globalCompositeOperation = "lighter";
  ctx.globalAlpha = 0.32 + Math.sin(state.elapsed * 12) * 0.05;
  drawPlayerEffectAt("treasureGlint", -size * 0.72, -size * 0.72, size * 1.44, size * 1.44);
  ctx.globalAlpha = 0.18;
  drawPlayerEffectAt("tidePulse", -size * 0.92, -size * 0.92, size * 1.84, size * 1.84);
  ctx.restore();
}
...
function drawWeaponEffects() {
  ...
  for (const zone of state.zones) {
    ...
    if (zone.type === "slash") {
    ...
    } else if (zone.type === "streetFighterExFlash") {
      ctx.translate(ox + zone.x, oy + zone.y);
      ctx.rotate(state.elapsed * 1.15);
      const burstSize = zone.radius * (1.45 + (1 - a) * 0.35);
      ctx.globalCompositeOperation = "lighter";
      drawPlayerEffectAt("treasureGlint", -burstSize * 0.52, -burstSize * 0.52, burstSize * 1.04, burstSize * 1.04);
      ctx.globalAlpha = a * 0.28;
      drawPlayerEffectAt("tidePulse", -burstSize * 0.72, -burstSize * 0.72, burstSize * 1.44, burstSize * 1.44);
      ...
      if (zone.signature === "dragonKick" && images.kenDragonFx) {
        drawKenDragonFxAt("dragonKick", Math.floor((1 - a) * KEN_DRAGON_FX.frames + state.elapsed * 8), -burstSize / 2, -burstSize / 2, burstSize, burstSize);
      } else {
...
function drawKenActionFrameAt(move, frame, x, y, w, h) {
  if (!images.kenActions) return;
  const row = kenActionRow(move);
  const sx = positiveModulo(frame || 0, KEN_ACTION.frames) * KEN_ACTION.w;
  const sy = row * KEN_ACTION.h;
  ctx.drawImage(images.kenActions, sx, sy, KEN_ACTION.w, KEN_ACTION.h, x, y, w, h);
}
...
function drawKenDragonFxAt(fx, frame, x, y, w, h) {
  if (!images.kenDragonFx) return;
  const src = kenDragonFxMap[fx] || kenDragonFxMap.dragonKick;
  const sx = positiveModulo(frame || 0, KEN_DRAGON_FX.frames) * KEN_DRAGON_FX.w;
  const sy = src.row * KEN_DRAGON_FX.h;
  ctx.drawImage(images.kenDragonFx, sx, sy, KEN_DRAGON_FX.w, KEN_DRAGON_FX.h, x, y, w, h);
}
...
function iconStyle(icon) {
  ...
  if (kenDragonFxMap[icon]) {
    const src = kenDragonFxMap[icon];
    const by = src.row / Math.max(1, KEN_DRAGON_FX.rows - 1) * 100;
    return `background-image:url('${imageSources.kenDragonFx}');background-size:${KEN_DRAGON_FX.frames * 100}% ${KEN_DRAGON_FX.rows * 100}%;background-position:0% ${by}%;`;
  }
...
window.__MONKEY_TIDE_CLEAR_PLAYTEST_AREA = (radius = 680) => {
  ...
  const gemsBefore = state.gems.length;
  ...
  state.gems = [];
  state.particles = state.particles.filter((particle) => Math.hypot((particle.x || 0) - p.x, (particle.y || 0) - p.y) >= radius);
  return { removed: removed.size, gemsRemoved: gemsBefore, particlesRemoved: particlesBefore - state.particles.length, debug: window.__MONKEY_TIDE_DEBUG() };
};
...
window.__MONKEY_TIDE_SIGNATURE_PROBE = (skinId = "ryu") => {
  ...
  state = makeState();
  state.phase = "playing";
  if (streetFighterSkinId(state.player.skin)) state.weapons.cutlass.level = 7;
...
window.__MONKEY_TIDE_KEN_ACTION_PROBE = () => {
  const savedState = state;
  const savedSkin = selectedSkin;
  const savedMuted = muted;
  selectedSkin = "ken";
  ...
  const result = {
    skin: state.player.skin,
    assetLoaded: !!images.kenActions,
    source: imageSources.kenActions,
    frames: { ...KEN_ACTION },
    dragonFxLoaded: !!images.kenDragonFx,
    dragonFxSource: imageSources.kenDragonFx,
    dragonFxFrames: { ...KEN_DRAGON_FX },
    rows: Object.fromEntries(Object.keys(KEN_ACTION.rowsByAction).map((action) => [action, kenActionRow(action)])),
    fxRows: Object.fromEntries(Object.keys(KEN_DRAGON_FX.rowsByFx).map((fx) => [fx, KEN_DRAGON_FX.rowsByFx[fx]])),
    ...
    enemiesDamaged: state.enemies.filter((enemy) => enemy.hp < enemy.maxHp).length,
    iconStyleUsesDragonSheet: ["shoryuken", "dragonKick", "dragonPunch", "stepKick", "whirlwindKick"].every((icon) => iconStyle(icon).includes(imageSources.kenDragonFx)),
  };
  state = savedState;
  selectedSkin = savedSkin;
  muted = savedMuted;
...
window.__MONKEY_TIDE_STREET_FIGHTER_IDLE_PROBE = () => {
  ...
  for (const skinId of ["ryu", "ken", "guile", "chunLi"]) {
    ...
    results[skinId] = {
      ...
      projectiles: state.projectiles.filter((projectile) => ["hadoken", "sonicBoom", "kiKouKen"].includes(projectile.signatureId)).map((projectile) => ({ id: projectile.signatureId, vx: Math.round(projectile.vx), vy: Math.round(projectile.vy) })),
...
window.__MONKEY_TIDE_KEN_CONTACT_FLOW_PROBE = () => {
  const savedState = state;
  const savedSkin = selectedSkin;
  const savedMuted = muted;
  selectedSkin = "ken";
  ...
  for (let i = 0; i < 3; i += 1) {
    ...
    const triggered = triggerStreetFighterContactCounter(enemy, dx, dy, distance);
    const zones = state.zones.slice(beforeZones).filter((zone) => ["kenStrike", "kenWhirlwind"].includes(zone.type)).map((zone) => ({
      type: zone.type,
      move: zone.move,
      fx: zone.fx,
      spinKick: !!zone.spinKick,
      radius: zone.radius,
    }));
    const projectiles = state.projectiles.slice(beforeProjectiles).filter((projectile) => projectile.signatureId === "hadoken").map((projectile) => ({
      id: projectile.signatureId,
      vx: Math.round(projectile.vx),
      vy: Math.round(projectile.vy),
      speed: Math.round(Math.hypot(projectile.vx, projectile.vy)),
      retarget: projectile.retarget,
    }));
    steps.push({
      triggered,
      count: idle.contactCounters.ken,
      action: state.playerAction ? { ...state.playerAction, row: kenActionRow(state.playerAction.type), frame: kenActionFrame(state.playerAction) } : null,
      zones,
      projectiles,
      contactDamage: enemy.hp < hpBefore,
    });
  }
  const result = {
    rows: Object.fromEntries(Object.keys(KEN_ACTION.rowsByAction).map((action) => [action, kenActionRow(action)])),
    ...
    steps,
    arsenal: { ...ensureKenArsenal() },
    contactCounters: { ...idle.contactCounters },
  };
  state = savedState;
  selectedSkin = savedSkin;
  muted = savedMuted;
...
window.__MONKEY_TIDE_STREET_FIGHTER_PEACEFUL_IDLE_PROBE = () => {
  const savedState = state;
  const savedSkin = selectedSkin;
  const savedMuted = muted;
  const results = {};
  ...
  const rowForSkin = {
    ryu: ryuActionRow,
    ken: kenActionRow,
    guile: guileActionRow,
    chunLi: chunLiActionRow,
  };
  for (const skinId of ["ryu", "ken", "guile", "chunLi"]) {
    selectedSkin = skinId;
    state = makeState();
    ...
    const profile = STREET_FIGHTER_BASIC_MOVEMENT[skinId];
    const config = actionConfigForSkin(skinId);
    const rowForAction = rowForSkin[skinId];
    const idle = ensureStreetFighterIdleState();
    idle.fireballTimers[skinId] = 99;
    const samples = [0.25, 1.1, 2.05, 3.2].map((elapsed) => {
      state.elapsed = elapsed;
      return basicMovementFrame(config, profile, rowForAction, false);
    });
    updateWeapons(1 / 60);
    const weaponSkipped = !state.playerAction && state.zones.length === 0 && state.projectiles.length === 0 && state.weapons.cutlass.timer > 0;
    idle.fireballTimers[skinId] = 0;
    state.elapsed = 8;
    updateStreetFighterIdleState(1 / 60);
    const projectile = state.projectiles.find((entry) => ["hadoken", "sonicBoom", "kiKouKen"].includes(entry.signatureId));
    results[skinId] = {
      peaceful: streetFighterPeacefulIdle(skinId),
      samples: samples.map((sample) => ({ action: sample.action, row: sample.row, frame: sample.frame, ambient: sample.ambient })),
      bounceOnly: samples.every((sample) => sample.action === "walk" && sample.row === rowForAction("walk") && !sample.ambient && STREET_FIGHTER_IDLE_FRAMES.includes(sample.frame)),
      weaponSkipped,
      fireballs: { ...ensureStreetFighterIdleState().fireballs },
      currentAction: state.playerAction ? { ...state.playerAction } : null,
...
window.__MONKEY_TIDE_STREET_FIGHTER_STARTER_KIT_PROBE = () => {
  const savedState = state;
  const savedSkin = selectedSkin;
  const savedMuted = muted;
  const results = {};
  muted = true;
  const moveFromProjectile = (projectile) => {
    if (projectile.signatureId === "hadoken") return "hadoken";
    if (projectile.signatureId === "sonicBoom") return "sonicBoom";
    if (projectile.signatureId === "kiKouKen") return "kiKouKen";
    return projectile.signatureId || projectile.icon || null;
  };
  for (const skinId of ["ryu", "ken", "guile", "chunLi"]) {
    selectedSkin = skinId;
    state = makeState();
    state.phase = "playing";
    state.weapons.cutlass.level = 1;
    state.weapons.cutlass.timer = 0;
    const p = state.player;
    for (let i = 0; i < 4; i += 1) {
      window.__MONKEY_TIDE_SPAWN_ENEMY("crab", p.x + 84 + i * 18, p.y + (i - 1.5) * 12, false);
    }
    updateWeapons(1 / 60);
    state.signatureMove.timer = 0;
    castSignatureMove(state.characterTrait.signature);
    ensureStreetFighterIdleState().contactCooldown = 0;
    window.__MONKEY_TIDE_SPAWN_ENEMY("crab", p.x + 8, p.y, false);
    updateEnemies(1 / 60);
    const moves = new Set();
    if (state.playerAction?.type) moves.add(state.playerAction.type);
    for (const zone of state.zones) {
      if (zone.move) moves.add(zone.move);
    }
    for (const projectile of state.projectiles) {
      const move = moveFromProjectile(projectile);
      if (move) moves.add(move);
    }
    const profile = STREET_FIGHTER_BASIC_MOVEMENT[skinId];
    results[skinId] = {
      starter: STREET_FIGHTER_STARTER_MOVES[skinId],
      moves: [...moves].sort(),
      startupActions: basicMovementSummary(actionConfigForSkin(skinId), profile).startupActions,
      allAmbientActions: [...profile.ambient],
      action: state.playerAction ? state.playerAction.type : null,
      zones: state.zones.map((zone) => ({ type: zone.type, move: zone.move || null })),
      projectiles: state.projectiles.map((projectile) => moveFromProjectile(projectile)).filter(Boolean),
      contactCounters: { ...ensureStreetFighterIdleState().contactCounters },
    };
  }
  state = savedState;
...
window.__MONKEY_TIDE_FIREBALL_STAGE_PROBE = () => {
  ...
  const savedMuted = muted;
  muted = true;
  function sample(skinId, level, fire) {
    selectedSkin = skinId;
    state = makeState();
    state.phase = "playing";
    state.weapons.cutlass.level = level;
    fire();
    const projectile = state.projectiles.find((entry) => ["hadoken", "sonicBoom", "kiKouKen"].includes(entry.signatureId));
    return projectile ? {
      skin: skinId,
      level,
      id: projectile.signatureId,
      stage: projectile.hadokenStage ?? projectile.sonicStage ?? projectile.kiKouKenStage ?? 0,
      size: projectile.size,
      radius: projectile.r,
      vx: Math.round(projectile.vx),
      vy: Math.round(projectile.vy),
  ...
  const result = {
    ryu: levels.map((level) => sample("ryu", level, () => fireRyuHadoken(level, 0, { exactAngle: true }))),
    guile: levels.map((level) => sample("guile", level, () => fireGuileSonicBoom(level, 0))),
    chunLi: levels.map((level) => sample("chunLi", level, () => fireSignatureProjectile(state.characterTrait.signature, 0))),
  };
  state = savedState;
  selectedSkin = savedSkin;
  muted = savedMuted;
...
window.__MONKEY_TIDE_STREET_FIGHTER_EX_FLOW_PROBE = () => {
  const savedState = state;
  const savedSkin = selectedSkin;
  const savedMuted = muted;
  const results = {};
  muted = true;
  const level = 7;
  const baseProjectileStats = (skinId) => {
    if (skinId === "guile") {
      const stage = sonicBoomStage(level);
      return {
        stage,
    ...
    if (skinId === "chunLi") {
      const stage = kiKouKenStage(level);
      return {
        stage,
        size: stagedProjectileValue(FIREBALL_STAGE_TUNING.kiKouKen.size, stage),
        radius: stagedProjectileValue(FIREBALL_STAGE_TUNING.kiKouKen.radius, stage),
    ...
    const stage = ryuHadokenStage(level);
    return {
      stage,
      size: stagedProjectileValue(FIREBALL_STAGE_TUNING.hadoken.size, stage) * (skinId === "ken" ? 0.94 : 1),
      radius: stagedProjectileValue(FIREBALL_STAGE_TUNING.hadoken.radius, stage) * (skinId === "ken" ? 0.92 : 1),
    };
  };
  for (const skinId of STREET_FIGHTER_FLOW_SKINS) {
    selectedSkin = skinId;
    state = makeState();
    state.phase = "playing";
    state.weapons.cutlass.level = level;
    const p = state.player;
    for (let i = 0; i < 8; i += 1) {
      window.__MONKEY_TIDE_SPAWN_ENEMY("crab", p.x + 260 + i * 38, p.y + (i % 3 - 1) * 46, false);
    }
    const signature = state.characterTrait.signature || characterTrait(skinId).signature;
    if (skinId === "ryu") {
      castRyuShoryuken(signature, 0);
      castRyuWhirlwindKick(signature, 0);
      fireRyuHadoken(level, 0, { signature, exactAngle: true });
    } else if (skinId === "ken") {
      castKenStepKick(level, 0, { exactAngle: true, followups: false, signature });
      castKenDragonPunch(signature, 0, { move: "shoryuken", fx: "shoryuken", level });
      fireKenHadoken(level, 0, signature, { exactAngle: true });
    } else if (skinId === "guile") {
      castGuileKneeBazooka(level, 0);
      castGuileReversePunch(level, 0);
      fireGuileSonicBoom(level, 0, { exactAngle: true, followups: false });
    } else if (skinId === "chunLi") {
      castChunLiThousandKick(level, 0);
      castChunLiWhirlwindKick(signature, 0);
      fireSignatureProjectile({ ...signature, id: "kiKouKen", icon: "kiKouKen" }, 0);
    }
    const projectile = state.projectiles.find((entry) => entry.ex) || state.projectiles.find((entry) => ["hadoken", "sonicBoom", "kiKouKen"].includes(entry.signatureId));
    const flow = ensureStreetFighterFlowState();
    const base = baseProjectileStats(skinId);
    results[skinId] = {
      base,
      chain: flow.chain[skinId],
      charges: flow.charges[skinId],
      totalEx: flow.totalEx[skinId],
      lastMove: flow.lastMove,
      readyMove: flow.readyMove,
      exFlash: state.zones.filter((zone) => zone.type === "streetFighterExFlash" && zone.skinId === skinId).length,
      projectile: projectile ? {
        id: projectile.signatureId,
        ex: !!projectile.ex,
        exMove: projectile.exMove || null,
        size: projectile.size,
        radius: projectile.r,
        speed: Math.round(Math.hypot(projectile.vx, projectile.vy)),
        vx: Math.round(projectile.vx),
...
window.__MONKEY_TIDE_DEBUG = () => {
  ...
  kenActionAsset: !!images.kenActions,
  kenActionSource: imageSources.kenActions,
  kenActionFrames: { ...KEN_ACTION },
  kenDragonFxAsset: !!images.kenDragonFx,
  kenDragonFxSource: imageSources.kenDragonFx,
  ...
  streetFighterBasicMovement: {
    ...
    guile: basicMovementSummary(GUILE_ACTION, STREET_FIGHTER_BASIC_MOVEMENT.guile),
    chunLi: basicMovementSummary(CHUN_LI_ACTION, STREET_FIGHTER_BASIC_MOVEMENT.chunLi),
  },
  ...
  playerSkinAnimated: (playerSkinMap[state.player.skin]?.animRow !== undefined && !!images.playerSkinWalks)
    ...
    || (playerSkinMap[state.player.skin]?.animSheet === "kenActions" && !!images.kenActions)
  ...
  streetFighterFlow: state.streetFighterFlow ? {
    chain: { ...ensureStreetFighterFlowState().chain },
    charges: { ...ensureStreetFighterFlowState().charges },
    totalEx: { ...ensureStreetFighterFlowState().totalEx },
    lastSkin: ensureStreetFighterFlowState().lastSkin,
    lastMove: ensureStreetFighterFlowState().lastMove,
    readyMove: ensureStreetFighterFlowState().readyMove,
  } : null,
  ...
  uiIconSources: {
    ...
    kenDragonIcons: ["shoryuken", "dragonKick", "dragonPunch", "stepKick", "whirlwindKick"].every((icon) => iconStyle(icon).includes(imageSources.kenDragonFx)),
  ...
  audio: {
    ...
    sfxMasterGain: SFX_MASTER_GAIN,
    sfx: Object.fromEntries(Object.keys(soundConfig).map((key) => [key, soundVolume(key)])),
    sfxRaw: Object.fromEntries(Object.entries(soundConfig).map(([key, config]) => [key, config.volume])),
    sfxDebug: { ...sfxDebug },
    ...
    sfxToMusicRatio: Number((Math.max(...Object.keys(soundConfig).map((key) => soundVolume(key))) / Math.max(0.01, music?.volume ?? 0, rushMusic?.volume ?? 0)).toFixed(2)),
  ...
  combatAssets: {
    ...
    kenActions: !!images.kenActions,
    kenDragonFx: !!images.kenDragonFx,
    ...
    streetFighterPeacefulIdleOnly: true,
    streetFighterIdleNearbyRadius: STREET_FIGHTER_IDLE_NEARBY_RADIUS,
    streetFighterStarterMoves: { ...STREET_FIGHTER_STARTER_MOVES },
    streetFighterExFlow: true,
    streetFighterExThreshold: STREET_FIGHTER_FLOW_THRESHOLD,
    streetFighterExMaxCharges: STREET_FIGHTER_FLOW_MAX_CHARGES,
    streetFighterExTuning: { ...STREET_FIGHTER_EX_TUNING },
    streetFighterBasicMovement: {
      ryu: basicMovementSummary(RYU_ACTION, STREET_FIGHTER_BASIC_MOVEMENT.ryu),
      ken: basicMovementSummary(KEN_ACTION, STREET_FIGHTER_BASIC_MOVEMENT.ken),
      guile: basicMovementSummary(GUILE_ACTION, STREET_FIGHTER_BASIC_MOVEMENT.guile),
    ...
    kenDragonFxTypes: Object.keys(kenDragonFxMap),
